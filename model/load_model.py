...
class load_model:
    ...
    def evaluate_pre_train(self, fold):
        ...
        data_loaders.dataset.__test__() # sets whole dataset to test mode means it doesn't augment images
        if S.train_line == False:
            evaluate_functions.performance_metrics(self.model_load,S.sigmas,S.gamma, self.epochs_completed, S.fold)
        elif S.train_line == True:
            evaluate_functions.performance_metrics_line(self.model_load,S.sigmas,S.gamma, self.epochs_completed, S.fold)

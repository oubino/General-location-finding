...
if S.downsample_user == True:
    ...
    trans_augment = transforms.Compose([T.Resize(S.in_z,S.in_x,S.in_y),T.Upsidedown_scipy(), T.Extract_landmark_location(), T.Fix_base_value(),T.Check_landmark_still_there('fix base'), T.Normalise(S.normal_min, S.normal_max, S.normal_window), T.Check_landmark_still_there('normalise'),T.Flips_scipy(),T.Check_landmark_still_there('flips scipy'), T.Horizontal_flip(), T.Check_landmark_still_there('horizontal flip'),T.Check_left_right(),T.Check_landmark_still_there('flip left right'), T.ToTensor()])

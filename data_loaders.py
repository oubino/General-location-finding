...
if S.downsample_user == True:
    trans_plain = transforms.Compose([T.Normalise(S.normal_min, S.normal_max, S.normal_window), T.Resize(S.in_z,S.in_x,S.in_y), T.Upsidedown_scipy(), T.ToTensor()])
    trans_augment = transforms.Compose([T.Normalise(S.normal_min, S.normal_max, S.normal_window),T.Resize(S.in_z,S.in_x,S.in_y), T.Upsidedown_scipy(), T.Flips_scipy(), T.Horizontal_flip(),T.ToTensor()])

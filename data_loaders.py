...
if S.downsample_user == True:
    ...
    trans_augment = transforms.Compose([T.Resize(S.in_z,S.in_x,S.in_y),T.Upsidedown_scipy(), T.Extract_landmark_location(), T.Fix_base_value(), T.Normalise(S.normal_min, S.normal_max, S.normal_window),  T.Flips_scipy(), T.Check_landmark_still_there('pre'), T.Horizontal_flip(),T.Check_landmark_still_there('post'),  T.Check_left_right(), T.ToTensor()])

...
if S.downsample_user == True:
    ...
    trans_plain = transforms.Compose([T.CentreCrop_test(S.in_z,S.in_x,S.in_y), T.Upsidedown_scipy(), T.Check_left_right(), T.Normalise_final(), T.ToTensor()])
    trans_augment = transforms.Compose([T.CentreCrop_train(S.in_z,S.in_x,S.in_y),T.Upsidedown_scipy(), T.Shift(), T.Flips_scipy(), T.Horizontal_flip(), T.Check_left_right(), T.Normalise_final(), T.Noise(), T.ToTensor()])
